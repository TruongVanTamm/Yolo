...
router.post('/upload', auth , authAdmin, (req, res) =>{
    try {
        if(!req.files || Object.keys(req.files).length === 0)
            return res.status(400).json({msg: 'Không có tệp tin nào được tải lên'})
        ...
        if(file.mimetype !== 'image/jpeg' && file.mimetype !== 'image/png'){
            removeTmp(file.tempFilePath)
            return res.status(400).json({msg: "Định dạnh file không hợp lệ"})
        }

        cloudinary.v2.uploader.upload(file.tempFilePath, {folder: "yolo"}, async(err, result)=>{
            if(err) throw err;
...
router.post('/destroyAll',auth , authAdmin, (req, res) =>{
    try {
        ...
        cloudinary.v2.uploader.destroy( public_id_2, async(err, result) =>{
            if(err) throw err;
